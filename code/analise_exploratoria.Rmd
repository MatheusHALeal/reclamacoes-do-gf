...
**Avaliaçoeso das reclamaçoes de um mesmo órgão**
```{r message=FALSE,  warning=FALSE}
avaliadores <- table(reclamacoes$orgao)
barplot(avaliadores ,main="Avaliação das reclamaçoes de um mesmo órgão", xlab="Órgão", ylab="Reclamaçoes", pch=19)
```
**Avaliaçoes de um mesmo individuo**
```{r message=FALSE,  warning=FALSE}
avaliadores <- table(avaliacoes$Matricula)
hist(avaliadores, main="Avaliaçoes de um mesmo individuo", xlab="Avaliaçoes", pch=19)
```
